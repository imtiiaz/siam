(function($){
	$(document).ready(function(){
		...
		$('form#student_add_form').submit(function(event){
			...
			let name = $('form#student_add_form input[name="name"]').val();
			let email = $('form#student_add_form input[name="email"]').val();
			let cell = $('form#student_add_form input[name="cell"]').val();			
			
			//file value
			let photo = $('form#student_add_form input[name="photo"]').val();

			//form validation
			if (name=='' || email=='' || cell=='') {
				$('.mess').html('<p class="alert alert-danger" >all fields are required<button class="close" data-dismiss="alert">&times;</button></p>');
			}else if(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)==false){
				$('.mess').html('<p class="alert alert-danger" >Invalid E-mail<button class="close" data-dismiss="alert">&times;</button></p>');
			}else{
				/**
